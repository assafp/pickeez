...
namespace '/albums' do
  ...
  #create
  post '/create' do
    data = params.merge!({owner_id: cuid})
    album_local_id = data.delete('album_local_id')
    res = Albums.create(data)    
    $albums.update_id(res[:_id], { "album_local_id.#{cuid}" => album_local_id } )
    {_id: res[:_id]}
  end
  ...
  post '/:id/set_album_local_id' do
    $albums.update_id(params[:id], { "album_local_id.#{cuid}" => params[:album_local_id] } )
    {msg: "ok"}
  end
  ...
  get '/algo/get_pending' do
    ...
    if pending_album
      ...
      photos = $photos.find_all({album_id: pending_id}).map { |p| p.just(:_id, :s3_path, :computed_filters, :filters, :num_faces, :rectangles, :detected_data, :owner_id, :algo_decision, :photo_creation_date, :local_creation_date) }
      {status: 'ok',
       album_id: pending_id,
       album: album,
